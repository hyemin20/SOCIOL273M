...
bal.plot(match_ps, var.name = "student_Trust", which = "both", type = "density")
bal.plot(match_ps, var.name = "student_Race", which = "both", type = "density")
bal.plot(match_ps, var.name = "student_GovtOpinion", which = "both", type = "density")
bal.plot(match_ps, var.name = "parent_EducHH", which = "both", type = "density")
bal.plot(match_ps, var.name = "parent_Money", which = "both", type = "density")
bal.plot(match_ps, var.name = "parent_Race", which = "both", type = "density")
bal.plot(match_ps, var.name = "parent_GovtOpinion", which = "both", type = "density")
...
colnames <- sample(names(ypsps_clean %>% select(-c(student_ppnscal, college))), 10)
independent <- paste(colnames, collapse = " + ")
ps_formula <- as.formula(paste("college ~", independent))
...
grid.arrange(grobs = list, ncol = 2, widths = c(5, 5), heights = c(2, 2, 2, 2, 2))
...
\begin{enumerate}
    ...
    Your Answer: Among 10,000 simulations, 169 resulted in models with a higher proportion of balanced covariates compared to the average proportion. This accounts for approximately 1.69% of the simulations, which is significantly lower than what one would expect in normal distribution. This discrepancy raises concerns about the robustness and reliability of the propensity scoring method. It suggests that the method may have a bias towards producing models with seemingly stronger covariate balance, potentially leading to inflated estimates of treatment effects. The distribution of the proportions also appears to be skewed, indicating potential issues with the model specification or data quality. Further investigation into the factors contributing to this imbalance and exploration of alternative modeling approaches may be warranted to improve the accuracy of treatment effect estimation.
...
# Fit p-score models and save ATTs, proportion of balanced covariates, and mean percent balance improvement
sim_results_r <- data.frame(ATTs, prop_balanced, percent_imp)
summary(sim_results_r)

meanprop_r <- mean(sim_results$prop_balanced, na.rm = TRUE)
sim_results %>% filter(prop_balanced > meanprop) %>% nrow()
...
Even in randomized or as-if-random designs, conducting matching can offer several advantages. While randomization aims to evenly distribute covariates between treatment groups, it may not always achieve perfect balance, especially with small sample sizes or many covariates. Matching provides a complementary approach to create more comparable treatment and control groups based on observed covariates, thereby reducing bias and improving the precision of treatment effect estimates. Additionally, matching allows researchers to explore and visualize covariate balance, providing insights into the effectiveness of randomization and potential sources of bias. Overall, matching serves as a valuable tool for robustness checks and enhancing the validity of causal inference in both randomized and observational studies.
